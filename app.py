...
def obtener_datos_filtrados(_engine, filtros_seleccionados):
    ...
    for key, value in filtros_seleccionados.items():
        if value is not None and value != "TODOS":
            ...
            # MODIFICACIÓN 2: Cambiado 'grupo' a 'cliente' para la clave del filtro
            if key == 'cliente': db_col_name = 'CLIENTE'
    ...
    if not df.empty:
        # MODIFICACIÓN 2: Se mantiene el renombramiento interno a GRUPO para consistencia de la app
...
def display_historical_analysis(df):
    ...
    # MODIFICACIÓN 5: Ajuste de decimales en todas las métricas
    with c1:
        st.markdown(f'<div class="metric-box"><h4>Precio Máximo Venta</h4><p>${max_venta:,.0f}</p></div>', unsafe_allow_html=True)
        st.markdown(f'<div class="metric-box"><h4>Precio de Venta</h4><p class="compound-text"><span class="compound-label">Promedio:</span>${mean_venta:,.0f}</p><p class="compound-text"><span class="compound-label">Moda:</span>${mode_venta:,.0f}</p></div>', unsafe_allow_html=True)
        st.markdown(f'<div class="metric-box"><h4>Precio Mínimo Venta</h4><p>${min_venta:,.0f}</p></div>', unsafe_allow_html=True)
    with c2:
        st.markdown(f'<div class="metric-box"><h4>Máx. Precio Reserva</h4><p>${max_reserva:,.0f}</p></div>', unsafe_allow_html=True)
        st.markdown(f'<div class="metric-box"><h4>Precio de Reserva</h4><p class="compound-text"><span class="compound-label">Promedio:</span>${mean_reserva:,.0f}</p><p class="compound-text"><span class="compound-label">Moda:</span>${mode_reserva:,.0f}</p></div>', unsafe_allow_html=True)
        st.markdown(f'<div class="metric-box"><h4>Mín. Precio Reserva</h4><p>${min_reserva:,.0f}</p></div>', unsafe_allow_html=True)
    with c3:
        st.markdown(f'<div class="metric-box"><h4>Recuperaciones Promedio</h4><p class="compound-text"><span class="compound-label">Venta/CC:</span>{recuperacion_venta_mean:.1%}</p><p class="compound-text"><span class="compound-label">Valor/EBC:</span>{recuperacion_valor_mean:.1%}</p></div>', unsafe_allow_html=True)
        st.markdown(f'<div class="metric-box"><h4>Costo Cliente Promedio</h4><p>${costo_cliente_mean:,.0f}</p></div>', unsafe_allow_html=True)
        st.markdown(f'<div class="metric-box"><h4>Valor de Mercado Promedio</h4><p>${mean_mercado:,.0f}</p></div>', unsafe_allow_html=True)
    with c4:
        st.markdown(f'<div class="metric-box"><h4>Actividad Promedio</h4><p class="compound-text"><span class="compound-label">Subastas:</span>{mean_subastas:.0f}</p><p class="compound-text"><span class="compound-label">Ofertas:</span>{mean_ofertas:.0f}</p></div>', unsafe_allow_html=True)
        st.markdown(f'<div class="metric-box"><h4>Días Hábiles de Venta Promedio</h4><p>{mean_dias:.0f}</p></div>', unsafe_allow_html=True)
...
<div style="background-color: white; padding: 10px; border-radius: 5px; text-align: center;">
    <a href="https://venturabbenchmarkinlga.streamlit.app/" target="_blank" style="text-decoration: none; font-weight: bold; color: black;">
        BENCHMARKING    </a>
</div>
''', unsafe_allow_html=True)
st.sidebar.divider()
...
# MODIFICACIÓN 3: Lógica del botón "Eliminar Filtros"
if st.sidebar.button("Eliminar Filtros"):
    st.session_state.analysis_run = False
    st.session_state.cliente_autenticado = None
    # Esto ahora buscará todas las claves que terminen en _widget y las reseteará
    keys_to_reset = [key for key in st.session_state if key.endswith('_widget')]
    for key in keys_to_reset:
        st.session_state[key] = 'TODOS'
    st.rerun()

# MODIFICACIÓN 3: Agregar claves a todos los selectbox
...
condicion_sel = st.sidebar.selectbox("Condición de Venta", ["TODOS"] + opciones_filtros['condiciones'], key='condicion_widget')
año_sel = st.sidebar.selectbox("Año del Modelo", ["TODOS"] + opciones_filtros['anios'], key='año_widget')
st.sidebar.markdown("---")

# --- Lógica de autenticación con secrets.toml de Streamlit ---
# MODIFICACIÓN 2: Cambiado "Grupo" a "Cliente" y variable "grupo_sel" a "cliente_sel"
cliente_sel = st.sidebar.selectbox("Cliente", ["TODOS"] + opciones_filtros['clientes'], key='cliente_widget')
...
if cliente_sel == "TODOS":
    ...
    if st.session_state.get("cliente_autenticado") == cliente_sel:
        st.sidebar.success(f"Acceso verificado para: {cliente_sel}")
        acceso_permitido = True
    else:
        st.sidebar.warning(f"Se requiere un código para acceder a los datos de '{cliente_sel}'.")
        client_code = st.sidebar.text_input("Código de acceso:", type="password", key=f"password_{cliente_sel}")

        if st.sidebar.button("Verificar Código", key=f"verify_{cliente_sel}"):
            try:
                ...
                cliente_sel_normalized = normalize_text(cliente_sel)

                if client_codes_normalized.get(cliente_sel_normalized) == client_code:
...
# MODIFICACIÓN 3: Agregar claves a todos los selectbox (continuación)
clas_venta_sel = st.sidebar.selectbox("Clasificación de Venta", ["TODOS"] + opciones_filtros['clas_venta'], key='clas_venta_widget')
clas_modelo_sel = st.sidebar.selectbox("Clasificación de Modelo", ["TODOS"] + opciones_filtros['clas_modelo'], key='clas_modelo_widget')
origen_marca_sel = st.sidebar.selectbox("Marca China", ["TODOS"] + opciones_filtros['origen_marca'], key='origen_marca_widget')
segmento_sel = st.sidebar.selectbox("Segmento", ["TODOS"] + opciones_filtros['segmentos'], key='segmento_widget')
combustible_sel = st.sidebar.selectbox("Combustible", ["TODOS"] + opciones_filtros['combustibles'], key='combustible_widget')
st.sidebar.markdown("---")
st.sidebar.header("Filtros por Fecha de Subasta")
año_subasta_sel = st.sidebar.selectbox("Año de Subasta", ["TODOS"] + opciones_filtros['anios_subasta'], key='año_subasta_widget')
meses_dict = {1: 'Enero', 2: 'Febrero', 3: 'Marzo', 4: 'Abril', 5: 'Mayo', 6: 'Junio', 7: 'Julio', 8: 'Agosto', 9: 'Septiembre', 10: 'Octubre', 11: 'Noviembre', 12: 'Diciembre'}
meses_nombres_disponibles = [meses_dict[m] for m in sorted(opciones_filtros['meses'])]
mes_subasta_sel_nombre = st.sidebar.selectbox("Mes de Subasta", ["TODOS"] + meses_nombres_disponibles, key='mes_subasta_widget')
...
if st.sidebar.button("Analizar / Estimar", disabled=(not acceso_permitido)):
    filtros_activos = {
        ...
        'cliente': cliente_sel, # MODIFICACIÓN 2: clave cambiada a 'cliente'
    ...
    if st.session_state.analysis_run:
        ...
        if df_filtrado_final.empty:
            ...
            if not df_filtrado_final.empty:
                ...
                # MODIFICACIÓN 5: Ajuste de decimales en treemap
                fig_treemap.update_traces(textinfo="label+percent parent", hovertemplate="<b>%{label}</b><br>Cantidad: %{value}<br>Porcentaje de la Marca: %{percentParent:.2%}<br>Porcentaje del Total: %{percentRoot:.2%}<br>Precio Promedio: $%{customdata[0]:,.0f}")
                ...
                # MODIFICACIÓN 5: Ajuste de decimales en format_volume
                format_volume = {
                    'Precio Promedio De Venta': '${:,.0f}', 
                    'Moda Del Precio De Venta': '${:,.0f}', 
                    'Precio Reserva Promedio': '${:,.0f}', 
                    'Moda Precio Reserva': '${:,.0f}', 
                    'Recuperacion Precio Prom': '{:.1%}', 
                    'Recuperacion Valor Prom': '{:.1%}', 
                    'Dias Habiles Promedio': '{:.0f}'
                }
            ...
            # MODIFICACIÓN 4: Agregar 'INVENTARIO' a la lista
            columnas_display = ['MARCA', 'MODELO', 'VERSION', 'AÑO', 'CONDICION_DE_VENTA', 'INVENTARIO', 'COMBUSTIBLE', 'CANTIDAD_OFERTADA', 'COSTO_CLIENTE', 'PRECIO_DE_MERCADO', 'RECUPERACION_PRECIO', 'RECUPERACION_VALOR', 'FECHA_DE_SUBASTA', 'DIAS_HABILES_VENTA', 'NUMERO_DE_SUBASTAS', 'NUMERO_DE_OFERTAS', 'SEGMENTO']
            ...
            # MODIFICACIÓN 4 & 5: Ajuste de decimales y agregado de 'Inventario'
            format_detalle = {
                'Cantidad Ofertada': '${:,.0f}', 
                'Costo Cliente': '${:,.0f}', 
                'Precio De Mercado': '${:,.0f}', 
                'Recuperacion Precio': '{:.1%}', 
                'Recuperacion Valor': '{:.1%}', 
                'Fecha De Subasta': '{:%Y-%m-%d}', 
                'Año': '{:.0f}',
                'Inventario': '{:.0f}' # Asumiendo que es numérico, sin decimales
            }
                ...
                with st.spinner("Calculando tendencias históricas..."):
                    ...
                    for col, title in cols_to_analyze.items():
                        ...
                        for v in df_evolucion[f'{col}_mean']:
                            if pd.isnull(v):
                                etiquetas.append('')
                            # MODIFICACIÓN 5: % ya estaba a 1 decimal
                            elif 'RECUPERACION' in col:
                                etiquetas.append(f"{v*100:.1f}%")
                            # MODIFICACIÓN 5: Días/Número ya eran int
                            elif 'DIAS' in col or 'NUMERO' in col:
                                etiquetas.append(f"{int(v)}")
                            # MODIFICACIÓN 5: Precios ya eran int
                            elif 'PRECIO' in col or 'CANTIDAD' in col or 'COSTO' in col or 'VALOR' in col:
                                etiquetas.append(f"${int(v):,}")
                            # MODIFICACIÓN 5: Caso 'else' ajustado a 0 decimales
                            else:
                                etiquetas.append(f"{v:.0f}")
                        ...
                        # MODIFICACIÓN 5: Formato de eje Y ya estaba correcto
    ...
    if st.session_state.analysis_run:
        ...
        if df_filtrado.empty:
            ...
            input_data.update({
                ...
                'GRUPO': df_filtrado['GRUPO'].mode().get(0, df_filtrado['GRUPO'].iloc[0]), # Se mantiene GRUPO internamente
            ...
            # Renombramiento de GRUPO a CLIENTE para el modelo
            ...
            # MODIFICACIÓN 5: Ajuste de decimales en cajas de pronóstico
            with p1: 
                st.markdown(f'''<div class="metric-box highlight">
                                <h4>Precio de Venta Estimado</h4>
                                <p class="compound-text"><span class="compound-label">Mínimo:</span>${min_precio:,.0f}</p>
                                <p class="compound-text"><span class="compound-label">Pronóstico:</span>${pred_precio:,.0f}</p>
            ...
            # MODIFICACIÓN 5: Ajuste de decimales en llamadas a generate_comparison_plots
            with v_tab1: generate_comparison_plots(df_filtrado, 'CANTIDAD_OFERTADA', pred_precio, modelo_precio, df_para_predecir, "Precio de Venta", "${:,.0f}")
            with v_tab2: generate_comparison_plots(df_filtrado, 'DIAS_HABILES_VENTA', pred_dias, modelo_dias, df_para_predecir, "Días de Venta", "{:.0f}")
            with v_tab3: generate_comparison_plots(df_filtrado, 'RECUPERACION_PRECIO', pred_recuperacion, modelo_recuperacion, df_para_predecir, "% Recuperación", "{:.1%}")
            with v_tab4: generate_comparison_plots(df_filtrado, 'NUMERO_DE_OFERTAS', pred_ofertas, modelo_ofertas, df_para_predecir, "# de Ofertas", "{:.0f}")
            st.markdown('</fieldset>', unsafe_allow_html=True)
            ...
            with st.expander("Ver Evaluación de Rendimiento del Modelo (Datos Filtrados)", expanded=True):
                ...
                def show_metrics_and_plot(tab, true_col, pred_col, title, format_str, format_str_delta):
                    with tab:
                        ...
                        m3.metric(label="Coeficiente de Determinación (R²)", value=f"{r2:.2%}") # R2 se mantiene con 2 decimales
                        st.markdown("---")
                ...
                # MODIFICACIÓN 5: Ajuste de decimales en llamadas a show_metrics_and_plot
                show_metrics_and_plot(m_tab1, 'CANTIDAD_OFERTADA', 'PRED_PRECIO', 'Precio de Venta', "${:,.0f}", "${:,.0f}")
                show_metrics_and_plot(m_tab2, 'DIAS_HABILES_VENTA', 'PRED_DIAS', 'Días de Venta', "{:.0f} días", "{:.0f} días")
                show_metrics_and_plot(m_tab3, 'RECUPERACION_PRECIO', 'PRED_RECUPERACION', '% Recuperación', "{:.1%}", "{:.1%}")
                show_metrics_and_plot(m_tab4, 'NUMERO_DE_OFERTAS', 'PRED_OFERTAS', '# de Ofertas', "{:.0f}", "{:.0f}")
    else:
