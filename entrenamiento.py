...
def cargar_y_procesar_datos():
    ...
    try:
        ...
        required_columns = ['MARCA', 'MODELO', 'VERSION', 'AÑO', 'MODELOT', 'Combustible', 'CONDICION DE VENTA', 'FECHA DE INGRESO', 'FECHA DE PAGO', 'NUMERO DE SUBASTAS', 'NUMERO DE OFERTAS', 'PRECIO RESERVA', 'CANTIDAD OFERTADA', 'COSTO CLIENTE', 'PRECIO DE MERCADO', 'ORIGEN', 'PROPIETARIO', 'FECHA DE SUBASTA', 'INVENTARIO']
        ...
        marcas_chinas = ["BAIC", "JMC", "CHANGAN", "DFSK", "JAC", "SERES", "MG", "CHIREY", "OMODA", "JAECOO", "JETOUR", "BYD", "SEV", "HAVAL", "ORA", "TANK", "GEELY", "ZEEKR", "GAC", "NETA", "BESTUNE", "FAW", "WEY", "KIRI", "IM", "EXCEED", "BAOJUN", "DENZA", "FENGON", "GEOMETRY", "FOTON", "MAXUS", "LYNK & CO", "ARRA", "KING LONG", "SKYWELL", "DONGFENG"]
...
AIG SEGUROS MEXICO	AIG	SALVAMENTOS
...
BTLC LATAM S. DE.RL DE C.V	BTLC LATAM	SEMINUEVOS
SOLVENTA SOLUCIONES AUTOMOTRICES	SOLVENTA	SEMINUEVOS
"""
        ...
        df_full['CLIENTE'] = df_full['CLIENTE'].fillna(df_full['PROPIETARIO'])
        df_full['SEGMENTO'] = df_full['SEGMENTO'].fillna("SEMINUEVOS")
        ...
        df_full['CANTIDAD_OFERTADA'] = pd.to_numeric(df_full['CANTIDAD_OFERTADA'], errors='coerce')
        df_full['COSTO_CLIENTE'] = pd.to_numeric(df_full['COSTO_CLIENTE'], errors='coerce')
        df_full['PRECIO_DE_MERCADO'] = pd.to_numeric(df_full['PRECIO_DE_MERCADO'], errors='coerce')
...
def entrenar_y_guardar_modelos(df_full):
    ...
    # <--- MODIFICACIÓN 1: CORREGIDO 'CANTIDAD_OFERTADE' A 'CANTIDAD_OFERTADA' ---
    ...
    columnas_a_quitar = ['CANTIDAD_OFERTADA', 'DIAS_HABILES_VENTA', 'RECUPERACION_PRECIO', 'NUMERO_DE_OFERTAS', 'RECUPERACION_VALOR', 'FECHA_DE_INGRESO', 'FECHA_DE_PAGO', 'FECHA_DE_SUBASTA', 'ORIGEN', 'PROPIETARIO', 'MODELOT', 'CONDICION_DE_VENTA', 'AÑO_SUBASTA', 'MES_SUBASTA', 'INVENTARIO']
    ...
    modelo_precio = Pipeline(steps=[('preprocesador', preprocesador),('regresor', RandomForestRegressor(n_estimators=34, random_state=9, n_jobs=-1))]); modelo_precio.fit(X, y_precio)
    modelo_dias = Pipeline(steps=[('preprocesador', preprocesador),('regresor', RandomForestRegressor(n_estimators=34, random_state=9, n_jobs=-1))]); modelo_dias.fit(X, y_dias)
    modelo_recuperacion = Pipeline(steps=[('preprocesador', preprocesador),('regresor', RandomForestRegressor(n_estimators=23, random_state=7, n_jobs=-1))]); modelo_recuperacion.fit(X, y_recuperacion)
    modelo_ofertas = Pipeline(steps=[('preprocesador', preprocesador),('regresor', RandomForestRegressor(n_estimators=34, random_state=9, n_jobs=-1))]); modelo_ofertas.fit(X, y_ofertas)
